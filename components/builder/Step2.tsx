...
export const Step2BackgroundAndDecorations: React.FC<{
  ...
  const activeFields = useMemo((): FormField[] => {
    // 1. Kiểm tra nếu mẫu hiện tại có cấu hình Form riêng từ Admin
    if (currentBg?.formFields && currentBg.formFields.length > 0) {
        return currentBg.formFields;
    }
    
    // 2. Nếu không có (mẫu cũ hoặc chưa cấu hình), hiển thị Form mặc định để không bị trống
    return [
  ...
  const handleBackgroundSelect = (bg: PresetBackground) => {
    ...
    // Khi đổi mẫu, ta KHÔNG xóa customFormData để nếu khách quay lại mẫu cũ vẫn còn dữ liệu
    // Nhưng ta sẽ cập nhật các thông tin cơ bản
    setConfig({ 
        ...config, 
        frameId: newFrameId,
                    ...
                    {field.type === 'image' && (
                        <div className="flex gap-3 items-center">
                            ...
                            >
                                ...
                                {config.customFormData?.[field.id] ? 'Đã tải ảnh' : 'Tải ảnh đính kèm'}
                            </button>
