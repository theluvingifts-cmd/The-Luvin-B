    ...
    const handlePrintOrder = () => {
        ...
        const html = `
            ...
            <body>
                ...
                <div class="info-grid">
                    <div class="box"><span class="box-title">Người nhận</span><p><strong>${selectedOrder.customer.name}</strong></p><p>${selectedOrder.customer.phone}</p><p>${selectedOrder.customer.address}${ (selectedOrder.customer.ward || selectedOrder.customer.district || selectedOrder.customer.province) ? `<br/><span style="font-size: 11px; color: #666;">${[selectedOrder.customer.ward, selectedOrder.customer.district, selectedOrder.customer.province].filter(Boolean).join(', ')}</span>` : '' }</p><p style="margin-top: 5px; font-style: italic;">Ghi chú: ${selectedOrder.delivery.notes || 'Không'}</p></div>
                        ...
                        <div className="flex-grow overflow-y-auto p-4 sm:p-6 space-y-6 sm:space-y-8">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div><h3 className="text-sm font-bold text-gray-900 border-b border-gray-200 pb-2 mb-3 uppercase tracking-wider">Khách hàng</h3><div className="space-y-2 text-sm text-gray-700">{isEditingOrder && editForm ? (<><div className="flex items-center gap-2"><span className="w-20 text-gray-500">Tên:</span> <input className="border rounded p-1 w-full" value={editForm.customer.name} onChange={e => handleEditFormChange('customer', e.target.value, 'name')} /></div><div className="flex items-center gap-2"><span className="w-20 text-gray-500">SĐT:</span> <input className="border rounded p-1 w-full" value={editForm.customer.phone} onChange={e => handleEditFormChange('customer', e.target.value, 'phone')} /></div><div className="flex items-center gap-2"><span className="w-20 text-gray-500">Email:</span> <input className="border rounded p-1 w-full" value={editForm.customer.email} onChange={e => handleEditFormChange('customer', e.target.value, 'email')} /></div>
                                        ...
                                        <div className="grid grid-cols-3 gap-2 mt-2 ml-20">
                                            <input className="border rounded p-1 text-[10px]" placeholder="Phường/Xã" value={editForm.customer.ward || ''} onChange={e => handleEditFormChange('customer', e.target.value, 'ward')} />
                                            <input className="border rounded p-1 text-[10px]" placeholder="Quận/Huyện" value={editForm.customer.district || ''} onChange={e => handleEditFormChange('customer', e.target.value, 'district')} />
                                            <input className="border rounded p-1 text-[10px]" placeholder="Tỉnh/TP" value={editForm.customer.province || ''} onChange={e => handleEditFormChange('customer', e.target.value, 'province')} />
                                        </div>
                                        ...
                                        <p className="flex items-start">
                                            <span className="text-gray-500 w-20 inline-block flex-shrink-0">Địa chỉ:</span> 
                                            <span className="text-gray-800">
                                                {selectedOrder.customer.address}
                                                {/* Chỉ hiển thị thêm nếu địa chỉ chính chưa bao gồm tỉnh/thành phố */}
                                                {selectedOrder.customer.province && !selectedOrder.customer.address.toLowerCase().includes(selectedOrder.customer.province.toLowerCase()) && (
                                                    <>
                                                        {', '}
                                                        {[selectedOrder.customer.ward, selectedOrder.customer.district, selectedOrder.customer.province].filter(Boolean).join(', ')}
                                                    </>
                                                )}
                                            </span>
                                        </p>
