...
export const AdminOrders: React.FC<AdminOrdersProps> = ({ orders, setOrders, products, frames, currentUser, role, onRefreshProducts }) => {
    ...
    const filteredOrders = useMemo(() => {
        ...
        if (orderSearch.trim()) {
            ...
            const termNoAccents = removeVietnameseTones(term);

            result = result.filter(o => {
                const idMatch = o.id.toLowerCase().includes(term);
                const phoneMatch = o.customer.phone.includes(term);
                
                const name = o.customer.name ? o.customer.name.toLowerCase() : '';
                const nameNoAccents = removeVietnameseTones(name);
                const nameMatch = name.includes(term) || nameNoAccents.includes(termNoAccents);
