...
export const AdminQuotation: React.FC<AdminQuotationProps> = ({ products, frames, config }) => {
    ...
    // Danh mục Combo B2B chia theo nhu cầu thực tế
    const quickPackages = [
        { group: 'Khung Vuông 15x15cm', items: [
            { name: 'Khung 15x15 + 1 Nhân vật', price: 230000, type: 'frame' },
            { name: 'Khung 15x15 + 2 Nhân vật', price: 280000, type: 'frame' },
        ]},
        { group: 'Khung Vuông 23x23cm', items: [
            { name: 'Khung 23x23 + 2 Nhân vật', price: 320000, type: 'frame' },
            { name: 'Khung 23x23 + 3 Nhân vật', price: 370000, type: 'frame' },
            { name: 'Khung 23x23 + 4 Nhân vật', price: 420000, type: 'frame' },
        ]},
        { group: 'Khung A5 (15x21cm)', items: [
            { name: 'Khung A5 + 1 Nhân vật', price: 240000, type: 'frame' },
            { name: 'Khung A5 + 2 Nhân vật', price: 290000, type: 'frame' },
        ]},
        { group: 'Hạng mục bổ sung (Add-ons)', items: [
            { name: 'Thêm 01 Nhân vật LEGO', price: 50000, type: 'part' },
            { name: 'Thêm 01 Charm / Phụ kiện', price: 15000, type: 'part' },
            { name: 'In mặt/áo theo yêu cầu', price: 150000, type: 'other' },
            { name: 'Hộp quà cao cấp B2B', price: 30000, type: 'other' },
        ]}
    ];
            ...
            <div className="lg:w-80 xl:w-96 flex-shrink-0 bg-white p-5 rounded-2xl border border-gray-200 shadow-sm space-y-6 print:hidden overflow-y-auto max-h-[85vh] sticky top-24 custom-scrollbar">
                <div className="flex items-center justify-between border-b pb-3">
                    <h3 className="text-lg font-bold text-gray-800 flex items-center gap-2">
                        <span className="text-xl">📄</span> Tạo báo giá B2B
                    </h3>
                </div>
                
                {/* 1. Quick Add Combo Section */}
                <div className="space-y-4">
                    {quickPackages.map((group, gIdx) => (
                        <div key={gIdx} className="space-y-2">
                            <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block">{group.group}</label>
                            <div className="grid grid-cols-1 gap-1.5">
                                {group.items.map((pkg, idx) => (
                                    <button 
                                        key={idx}
                                        onClick={() => addItem(pkg)}
                                        className="text-left p-2 bg-gray-50 hover:bg-pink-50 border border-gray-200 hover:border-pink-300 rounded-xl transition-all flex justify-between items-center group"
                                    >
                                        <div className="flex flex-col min-w-0">
                                            <span className="text-[11px] font-bold text-gray-700 truncate">{pkg.name}</span>
                                            <span className="text-[10px] text-gray-400">{formatCurrency(pkg.price)}</span>
                                        </div>
                                        <span className="text-primary font-bold text-lg opacity-0 group-hover:opacity-100 transition-opacity">+</span>
                                    </button>
                                ))}
                            </div>
                        </div>
                ...
                <div className="space-y-3 pt-4 border-t">
                    ...
                    <input className="w-full p-2.5 border rounded-xl text-sm focus:ring-1 focus:ring-pink-400 outline-none" placeholder="Tên khách hàng" value={quote.customerName} onChange={e => setQuote({...quote, customerName: e.target.value})} />
                    <input className="w-full p-2.5 border rounded-xl text-sm focus:ring-1 focus:ring-pink-400 outline-none" placeholder="Tên công ty" value={quote.companyName} onChange={e => setQuote({...quote, companyName: e.target.value})} />
                    <input className="w-full p-2.5 border rounded-xl text-sm focus:ring-1 focus:ring-pink-400 outline-none" placeholder="Số điện thoại" value={quote.phone} onChange={e => setQuote({...quote, phone: e.target.value})} />
                </div>

                {/* 3. Items Management */}
                <div className="space-y-3">
                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block">Danh sách đã chọn ({quote.items.length})</label>
                    <div className="space-y-2">
                        {quote.items.map(item => (
                            <div key={item.id} className="p-3 bg-white rounded-xl border border-gray-100 shadow-sm space-y-2 group">
                                <div className="flex justify-between items-start gap-2">
                                    <input 
                                        className="font-bold text-[11px] text-gray-800 bg-transparent border-b border-transparent focus:border-pink-300 outline-none w-full"
                                        value={item.name}
                                        onChange={(e) => updateItemName(item.id, e.target.value)}
                                    />
                                    <button onClick={() => removeItem(item.id)} className="text-red-400 hover:text-red-600 font-bold">×</button>
                                </div>
                                <div className="flex gap-2">
                                    <div className="w-16">
                                        <label className="text-[8px] text-gray-400 uppercase font-bold">SL</label>
                ...
                <div className="pt-4 border-t space-y-3">
                    ...
                    <div>
                        <label className="text-[10px] font-bold text-gray-500">Phí vận chuyển</label>
                    ...
                    <textarea className="w-full p-2.5 border rounded-xl text-[11px] h-24 bg-gray-50 italic" value={quote.note} onChange={e => setQuote({...quote, note: e.target.value})} />
                </div>

                <button onClick={() => window.print()} className="w-full bg-primary text-white font-bold py-3 rounded-2xl hover:brightness-95 shadow-lg shadow-pink-100 transition-all flex items-center justify-center gap-2">
                    ...
                    <div className="flex justify-between items-start mb-12 border-b border-gray-100 pb-8">
                        ...
                        <div className="text-right">
                            ...
                            <div className="text-[11px] text-gray-600 font-medium">
                                ...
                                <p><span className="text-gray-400">Hiệu lực:</span> 15 ngày kể từ ngày lập</p>
                            </div>
                        </div>
                    </div>
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="border-y border-gray-200 bg-gray-50/50">
                                    ...
                                    <th className="py-4 px-2 text-[10px] font-bold uppercase text-gray-400">Nội dung sản phẩm / Dịch vụ</th>
                                ...
                                {quote.items.length > 0 ? quote.items.map((item, idx) => (
                                    <tr key={item.id}>
                                        ...
                                        <td className="py-5 px-2">
                                            ...
                                            <p className="text-[10px] text-gray-400 italic">Hạng mục: {item.type === 'frame' ? 'Combo khung hoàn thiện' : 'Linh kiện/Dịch vụ lẻ'}</p>
                                        </td>
                                    ...
                                    <tr>
                                        <td colSpan={5} className="py-24 text-center text-gray-300 italic text-sm">Vui lòng chọn các gói Combo hoặc linh kiện ở bảng bên trái.</td>
                                    </tr>
                                )}
                            </tbody>
                        </table>
                    ...
                    <div className="mt-12 pt-8 border-t-2 border-gray-50 flex flex-col md:flex-row justify-between items-start gap-12">
                        ...
                        <div className="w-full md:w-72 bg-gray-50/50 p-6 rounded-2xl border border-gray-100 space-y-3">
                            ...
                            <div className="border-t border-gray-200 pt-3 mt-1 flex justify-between items-center">
                                <span className="text-sm font-bold text-gray-900 uppercase tracking-tighter">Tổng cộng:</span>
