...
export const StoryRenderer: React.FC<StoryRendererProps> = ({
    ...
    const basePrice = adj.customPrice || (() => {
        ...
        if (!price) price = 290000;
        if (t.name?.toLowerCase().includes('bảo tàng')) price = 310000;
        return price;
    })();

    const isGallery = (t.productLine || t.config?.productLine) === 'gallery';
    const items = t.config?.draggableItems || [];
    const statsLabel = isGallery 
        ? `${items.filter(i => i.frameUrl).length || t.galleryOptions?.photoFrameCount || 1} khung ảnh`
        : `${(items.length + (t.config?.characters?.reduce((acc, char) => acc + (char.hat ? 1 : 0) + (char.set ? 1 : 0), 0) || 0))} charm`;
                        ...
                        {!adj.hideSpecs && (
                            <div className="absolute -left-12 -bottom-2 rotate-[-3deg] bg-white border-[4px] border-black p-8 shadow-2xl z-20">
                                ...
                                <div className="flex items-center gap-4">
                                    <span className="text-xl font-bold bg-black text-white px-4 py-1.5">{t.config?.characters?.length || 0} Figures</span>
                                    <span className="text-xl font-bold text-gray-500 uppercase tracking-tighter">{statsLabel}</span>
                                </div>
                            </div>
                        )}
                    </div>
                            ...
                            <div className="flex flex-col gap-8">
                                <div className="flex gap-12">
                                    ...
                                    <div className="flex flex-col">
                                        ...
                                        <span className="text-3xl font-black" style={{ color: textColor }}>{statsLabel}</span>
                                    </div>
                                </div>
                    ...
                    <div className="pt-10 pb-6 px-4 flex flex-col items-center text-center transition-transform duration-200" style={{ transform: `translateY(${adj.contentY}px)` }}>
                        ...
                        {!adj.hideSpecs && (
                            <div className="flex items-center gap-6 mt-8">
                                ...
                                <div className="flex items-center gap-2">
                                    <div className="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center text-lg">✨</div>
                                    <span className="text-xl font-bold text-gray-600">{statsLabel}</span>
                                </div>
                            </div>
                        )}
