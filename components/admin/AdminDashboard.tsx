...
const ConversionFunnel = ({ stats }: { stats: any }) => {
    ...
    // Lấy giá trị lớn nhất (thường là bước 1) để tính %
    ...
    return (
        <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm h-full">
            <h4 className="font-bold text-sm text-gray-700 uppercase tracking-wider mb-6 flex justify-between items-center">
                <span className="flex items-center gap-2">📊 Phễu chuyển đổi</span>
                <span className="text-[10px] bg-blue-50 text-blue-600 px-2 py-0.5 rounded">Theo bộ lọc</span>
            </h4>
                        ...
                        <div key={step.key} className="group">
                            ...
                            <div className="w-full h-2.5 bg-gray-100 rounded-full overflow-hidden flex relative">
                                <div 
                                    ...
                                    style={{ width: `${percent}%` }}
                                ></div>
                                {idx > 0 && dropRate > 5 && val > 0 && (
                                    <div className="absolute right-0 top-0 h-full bg-red-500/10 flex items-center px-1">
            ...
            <p className="mt-6 text-[10px] text-gray-400 italic leading-relaxed">
                * Dữ liệu hiển thị dựa trên bộ lọc ngày bạn đã chọn phía trên. Giúp bạn phân tích hiệu quả chiến dịch theo thời điểm.
            </p>
        </div>
    );
};
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ orders, products, frames }) => {
    ...
    // Tính toán Start/End date dựa trên bộ lọc
