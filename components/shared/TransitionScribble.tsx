...
export const TransitionScribble: React.FC = () => {
    ...
    useEffect(() => {
        // Find all logo-truus elements on the page as click triggers (e.g. Header, Footer)
        const logos = document.querySelectorAll('.logo-truus');
        ...
        const runScribbleAnimation = (e: Event | null) => {
            ...
            // PHASE 3: Continue path movement outwards to bottom-right to clear screen (keep max strokeWidth to prevent holes/gaps)
            drawTl.to(transitionScribblePath, { strokeDashoffset: -l - 50, duration: durOut, ease: 'power2.inOut' }, durIn);
        ...
        // Bind clicks on all matched navbar/footer logo elements
        logos.forEach(logo => {
            logo.addEventListener('click', runScribbleAnimation);
        });
        ...
        return () => {
            logos.forEach(logo => {
                logo.removeEventListener('click', runScribbleAnimation);
            });
            clearTimeout(timer);
        };
    }, []);
