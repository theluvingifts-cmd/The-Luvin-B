...
const FramePreview = React.forwardRef<HTMLDivElement, FramePreviewProps>(({ 
  ...
  const selectedDraggableType = useMemo(() => {
    ...
    const item = (Array.isArray(config.draggableItems) ? config.draggableItems : []).find(i => i.id === id);
  ...
  const activeColors = useMemo(() => {
      ...
      if (type === 'character' && activePartType) {
          const char = (Array.isArray(config.characters) ? config.characters : []).find(c => c.id === id);
          if (!char) return null;
  ...
  const getActiveColorHex = useMemo(() => {
      ...
      const id = parseInt(idStr);
      if (type === 'item') {
          return (Array.isArray(config.draggableItems) ? config.draggableItems : []).find(i => i.id === id)?.selectedColor?.hex;
      }
      if (type === 'character' && activePartType) {
          const char = (Array.isArray(config.characters) ? config.characters : []).find(c => c.id === id);
          if (!char) return null;
