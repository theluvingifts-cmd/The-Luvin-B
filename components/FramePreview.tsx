...
const LegoCharacter: React.FC<{ character: LegoCharacterConfig; pxPerCm: number }> = ({ character, pxPerCm }) => {
  ...
  const hairImageUrl = character.selectedHairColor?.imageUrl || character.hair?.imageUrl;
  const activeHeadwearUrl = character.hat?.imageUrl || hairImageUrl;
  const activeHeadwearName = character.hat?.name || character.hair?.name;
  ...
  return (
    <div style={containerStyle}>
      ...
      */}
      ...
      {activeHeadwearUrl && (
        <SafeImage src={activeHeadwearUrl} alt={activeHeadwearName || 'headwear'} style={{ ...partStyle, zIndex: 4 }} />
      )}
    </div>
  );
};
