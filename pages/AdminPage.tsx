...
const AdminPage: React.FC = () => {
    ...
    return (
        <div className="min-h-screen bg-gray-50 font-sans text-gray-900">
                                        ...
                                        <div>
                                            ...
                                            <div className="grid grid-cols-1 gap-4">
                                                {(isEditingOrder && editForm ? editForm.items : selectedOrder.items).map((item, idx) => {
                                                    // Filter original assets uploaded by user for this item
                                                    ...
                                                    <div key={idx} className="flex gap-4 border border-gray-100 rounded-lg p-4 items-start bg-white flex-col md:flex-row">
                                                        <div className="w-24 h-24 bg-gray-50 rounded border border-gray-200 flex-shrink-0 overflow-hidden flex items-center justify-center relative group cursor-pointer" onClick={() => item.previewImageUrl && setZoomedImageUrl(item.previewImageUrl)}>
                                                            ...
                                                            <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                                                <span className="text-white text-[10px] font-bold bg-black/40 px-2 py-0.5 rounded">Phóng to</span>
                                                            </div>
                                                        </div>
                                                        <div className="flex-grow w-full">
                                                            ...
                                                            {/* CUSTOMER RESOURCES SECTION - HIGHLIGHTED */}
                                                            {customerAssets.length > 0 && (
                                                                <div className="mb-6 p-4 bg-blue-50 border border-blue-100 rounded-xl shadow-sm">
                                                                    ...
                                                                    <div className="flex flex-wrap gap-4">
                                                                        {customerAssets.map((asset, aIdx) => (
                                                                            <div key={aIdx} className="flex flex-col items-center gap-2 bg-white p-2 rounded-lg border border-blue-200 shadow-sm group">
                                                                                <div className="w-16 h-16 bg-gray-100 rounded border overflow-hidden cursor-pointer relative" onClick={() => setZoomedImageUrl(asset.url)}>
                                                                                    <img src={asset.url} alt="customer resource" className="w-full h-full object-cover" />
                                                                                    <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center text-white text-[8px] font-bold">XEM</div>
                                                                                </div>
                                                                                <div className="flex flex-col items-center">
                                                                                    <span className="text-[9px] font-black text-gray-500 uppercase mb-1">{asset.type}</span>
                                                                                    <button 
                                                                                        onClick={() => downloadImage(asset.url, `TL_${selectedOrder.id}_Item${idx+1}_${asset.type}.png`)}
                                                                                        className="text-[10px] bg-blue-600 text-white px-3 py-1 rounded-full font-bold hover:bg-blue-700 shadow-sm active:scale-95 transition-all"
                                                                                    >
                                                                                        Tải về
                                                                                    </button>
                                                                                </div>
            ...
            {/* Zoom Overlay */}
            {zoomedImageUrl && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 p-4 animate-fade-in" onClick={() => setZoomedImageUrl(null)}>
