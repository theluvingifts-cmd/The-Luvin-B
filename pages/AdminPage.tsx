...
import { AdminQuotation } from '../components/admin/AdminQuotation'; // Thêm mới

type MainTab = 'dashboard' | 'orders' | 'products' | 'config' | 'marketing' | 'customers' | 'design' | 'quotation';
...
const AdminPage: React.FC = () => {
    ...
    const role: StaffRole | null = useMemo(() => {
        ...
        if (SUPER_ADMINS.includes(currentUser.email) || currentUser.email.includes('admin')) return 'admin';
    ...
    return (
        <div className="min-h-screen bg-gray-50 font-sans text-gray-900">
                    ...
                    <div className="h-14 sm:h-16 flex justify-between items-center">
                        <div className="flex items-center gap-4 lg:gap-8">
                            ...
                            <nav className="hidden md:flex gap-1 overflow-x-auto no-scrollbar">
                                 {role === 'admin' && <button onClick={() => setActiveTab('dashboard')} className={`px-3 py-1.5 rounded-lg text-sm font-bold transition-all ${activeTab === 'dashboard' ? 'bg-gray-100 text-gray-900' : 'text-gray-500 hover:text-gray-800 hover:bg-gray-50'}`}>Dashboard</button>}
                                ...
                                {role === 'admin' && (
                                    <>
                                        <button onClick={() => setActiveTab('products')} className={`px-3 py-1.5 rounded-lg text-sm font-bold transition-all ${activeTab === 'products' ? 'bg-gray-100 text-gray-900' : 'text-gray-500 hover:text-gray-800 hover:bg-gray-50'}`}>Sản phẩm</button>
                                        ...
                                        <button onClick={() => setActiveTab('quotation')} className={`px-3 py-1.5 rounded-lg text-sm font-bold transition-all ${activeTab === 'quotation' ? 'bg-gray-100 text-gray-900' : 'text-gray-500 hover:text-gray-800 hover:bg-gray-50'}`}>Báo giá B2B</button>
            ...
            <main className="max-w-[1600px] mx-auto py-4 sm:py-8 px-2 sm:px-6">
                ...
                {activeTab === 'config' && role === 'admin' && <AdminConfig storeConfig={storeConfig} setStoreConfig={setStoreConfig} feedbacks={feedbacks} onRefreshFeedbacks={async () => setFeedbacks(await getAllFeedbacks())} />}
                ...
                {activeTab === 'quotation' && role === 'admin' && <AdminQuotation products={products} frames={frames} config={storeConfig} />}
            </main>
        </div>
    );
};
