...
import { getRecentOrders, incrementGlobalOrderCount } from '../services/orderService';
...
export const BuilderPage: React.FC<BuilderPageProps> = ({ config, setConfig, navigateTo, onAddToCart, onUpdateCart, showToast, legoParts, backgrounds, frames, editingCartIndex, onCancelEdit, onZoomImage, logoUrl, isEditingOrder, uploadedFonts, isLoadingParts, templates, storeConfig }) => {
  const { t } = useLanguage();
  ...
  const { totalPrice, priceBreakdown } = useMemo(() => calculatePrice(config, allParts, frames, templates, config.templateId), [config, allParts, frames, templates]);
    ...
    try {
        ...
        if (andCheckout) {
            if (config.templateId) {
                incrementTemplatePurchaseCount(config.templateId, 1);
            }
  ...
  const renderStepContent = () => {
    switch (step) {
      ...
      case 4: return <Step4Summary 
        ...
        productLine={config.productLine || 'lego'} 
